...
import LSTM.LoadBertData as LoadBertData
import LSTM.total_bert_model as totalmodel
import utils
from calculatePrecision import getLen
from LSTM.total_bert_model import BERTTotalModel
...
def trainOneModel(
    ...
    trainDataset = LoadBertData.LSTMDataSet(trainSeqs, eval=False)
    testDataset = LoadBertData.LSTMDataSet(testSeqs)

    trainDataLoader = LoadBertData.LSTMDataLoader(trainDataset)
    testDataloader = LoadBertData.LSTMDataLoader(testDataset)
    ...
    for i in range(args.nepoch):
        ...
        if i % args.testEvery == (args.testEvery - 1):
            ...
            for key in trainSeqs_keys:  # do evaluation for every key respectively
                predicts = []
                evalSeqs = LoadBertData.getSeqsFromKeys(key)
                evalDataSet = LoadBertData.LSTMDataSet(evalSeqs)
                evalDataloader = LoadBertData.LSTMDataLoader(evalDataSet)
...
def main():
    ...
    LoadBertData.loadFeatures()

    train_test_Seqs = LoadBertData.generateTrainAndTest(args.foldNum)
    # level has 1,2,3 each level we train foldNum models
    LSTMModels = [BERTTotalModel(args) for i in range(args.foldNum)]
    level = args.level
    ...
    for index, model in enumerate(LSTMModels):
        ...
        trainSeqs = LoadBertData.getSeqsFromKeys(trainSeqs_keys)
        testSeqs = LoadBertData.getSeqsFromKeys(testSeqs_keys)
        p.apply_async(
            trainOneModel,
            args=(
                args,
